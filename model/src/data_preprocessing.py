...
data['location_district'] = data['location'].apply(lambda x: x[1] if isinstance(x, list) and len(x) > 1 else None)
...
data = data[data.groupby('location_district')['location_district'].transform('count') > 1]

# Podział na train i test (stratyfikacja względem dzielnicy)
train, test = train_test_split(data, test_size=0.2, random_state=42, stratify=data['location_district'])
...
def preprocess_data(data):
    ...
    data.loc[:, 'build_year'] = data['build_year'].apply(lambda x: np.nan if x < 1000 or x > 2030 else x)
    ...
    data.loc[:, "building_floors"] = data.groupby("location_district")["building_floors"].transform(
        lambda x: x.fillna(x.mode()[0] if not x.mode().empty else data["building_floors"].mode()[0])
    )

    # Przetwarzanie liczby pokoi
    data.loc[:, "rooms"] = data["rooms"].apply(lambda x: int(x) if str(x).isdigit() else None)

    # Przetwarzanie liczby pokoi
    data.loc[:, "rooms"] = data["rooms"].apply(lambda x: int(x) if str(x).isdigit() else None)

    # Wypełnianie brakujących wartości na podstawie mediany dla każdego zakresu powierzchni
    data.loc[:, 'rooms'] = data.groupby(pd.cut(data['area'], bins=10), observed=True)['rooms'].transform('median')
    ...
    if data['rooms'].isna().sum() > 0:
        ...
        data.loc[:, "rooms"] = data["rooms"].fillna((data["area"] / avg_room_size).round())
    ...
    data['rent'] = data.apply(
        lambda row: row['rent'] if (
                    min_rent_per_m2 * row['area'] < row['rent'] < max_rent_per_m2 * row['area']) else 10 * row['area'],
        axis=1
    )
    ...
    data.loc[:, 'build_year'] = data['build_year'].round()  # Zaokrąglanie do pełnej liczby

    # Uzupełnianie brakujących cen na podstawie ceny za m²
    grouped = data.dropna(subset=['price', 'area']).groupby('location_district').agg(
        total_price=('price', 'sum'),
        total_area=('area', 'sum')
    ).reset_index()
    grouped['avg_price_m2'] = grouped['total_price'] / grouped['total_area']
    data = data.merge(grouped[['location_district', 'avg_price_m2']], on='location_district', how='left')

    data['price'] = data.apply(
        lambda row: row['avg_price_m2'] * row['area'] if pd.isna(row['price']) and not pd.isna(
            row['area']) else row['price'], axis=1)
    ...
    data['price_m2'] = data.apply(
        lambda row: np.nan if (row['price_m2'] < Q1 - 1.5 * IQR) or (row['price_m2'] > Q3 + 1.5 * IQR) else row['price_m2'],
        axis=1
    )
    ...
    utilities_df = pd.DataFrame(utilities_encoded, columns=['utilities_' + col for col in mlb.classes_])
    data = data.join(utilities_df)

    categorical_features = ['heating', 'state', 'market', 'ownership', 'ad_type','location_district']
